...
describe('DistrictRepository iteration 1 - part 1', () =>  {
  ...
  xtest('findByName returns undefined if no arguments are provided', () => {
    expect(district.findByName()).toBe(undefined);
  });

  xtest('findByName returns undefined when there are no matching arguments', () => {
    expect(district.findByName('Vikings')).toBe(undefined);
  });

  xtest('findByName returns an object with its individual district information', () => {

    expect(typeof district.findByName('Colorado')).toEqual('object');
    expect(district.findByName('Colorado').location).toEqual('COLORADO');
  });

  xtest('findByName search is not case sensitive', () => {
    expect(district.findByName('ColoRAdo').location).toEqual('COLORADO');
    expect(district.findByName('ACADEmY 20').location).toEqual('ACADEMY 20');
  });

  xtest('each district has a data object containing each year and its data as key value pairs.', () => {
  ...
  xtest('district data is rounded to the nearest hundredth', () => {
  ...
  xtest('district data is sanitized and defaults to 0', () => {
